...
import { expensesDataAddAPI, gstOptionsGetAPI, partyDataGetAPI, productGetAPI, unitsDataGetAPI } from "../../service/api/admin";
const other = {
    autoHeight: true,
    showCellVerticalBorder: true,
    showColumnVerticalBorder: true,
  };
function ExpenceTable({rows,setRows}) {
 
      const [productOptions, setProductOptions] = useState([]);
      const [unitOptions, setUnitOptions] = useState([]);

      const [selectedProduct, setSelectedProduct] = useState(null); // State to hold selected product
      const [selectedUnit, setSelectedUnit] = useState(null); // State to hold selected product
...
const unitFetch=()=>{
    unitsDataGetAPI().then((data)=>{
      ...
      const handleSelectedUnitChange= async (event, newValue) => {
      ...
      const handleTaxValueChange = (id,name, value) => {
       console.log("handleTaxValueChange",value)
        setRows(prevRows =>
          prevRows.map(row =>
            row.id === id ? { ...row, [name]: value } : row
          )
...
/>
        ...
        { field: 'unit', headerName: 'Unit', hideable: false, flex: 1,renderCell: (params) => (
            params.row.id === 'total' ?
            "":
            <Box sx={{ width: "100%", marginBottom: "10px" }}>
  ...
  />
                ...
                >
                  ...
                  <div style={{ flex: "1", textAlign: "center" }}>
                    <input
                      ...
                      type="text"
                      value={((params.row.qty || 0) * (params.row.price || 0) * ((params.row.discount || 0) / 100))}
                      disabled
                    />
                  </div>
                </div>
...

        {
            ...
            const discountedAmount= ((params.row.qty || 0) * (params.row.price || 0) * ((params.row.discount || 0) / 100)).toFixed(2)
const discountedTotal = ((params.row.qty || 0) * (params.row.price || 0)  -  discountedAmount)
const taxAmount = discountedTotal * ( taxValue/100)
                ...
                params.row.id === 'total' ? 
                  ...
                  <div style={{ width: "100%", display: "flex" }}>
                    <select
                      ...
                      onChange={(e) => handleTaxValueChange(params.row.id, "taxvalueinpercentage", e.target.value)}
                      style={{
                        width: "100%",
                        padding: "8px",
                        border: "none",
            ...
            
          {
            ...
            renderCell: (params) => {
              const discountedAmount = ((params.row.qty || 0) * (params.row.price || 0) * ((params.row.discount || 0) / 100)).toFixed(2);
              const discountedTotal = ((params.row.qty || 0) * (params.row.price || 0)) - discountedAmount;
              const taxValue = parseFloat(params.row.taxvalueinpercentage?.replace("%", "") || 0);
              const taxAmount = discountedTotal * (taxValue / 100);
